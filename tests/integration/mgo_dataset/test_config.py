...
from maceh import config
from maceh.response import constants

CFG_PATH = "workflows/mgo_dataset/configs/mgo.yaml"
...
def test_cli_unknown_stage_fails():
    r = subprocess.run([sys.executable, "-m", "workflows.mgo_dataset", "no-such-stage",
                        "--workspace", "/tmp/x"], capture_output=True)
    assert r.returncode != 0


def test_cli_help_lists_stages():
    r = subprocess.run([sys.executable, "-m", "workflows.mgo_dataset", "--help"],
