from maceh.data.io.blocks import atom_u, orbital_u, transform_block
import numpy as np
...
def test_p_transform_hand_checked():
    ...
    assert np.allclose(orbital_u(1) @ v, [-2.0, -3.0, 1.0])
...
def test_d_transform_hand_checked():
    ...
    assert np.allclose(orbital_u(2) @ v, [0.0, 3.0, 4.0, -1.0, -2.0])
...
def test_atom_u_block_diagonal():
    ...
    assert np.allclose(u[2:, 2:], orbital_u(1))
...
def test_transform_preserves_hermitian_pairs():
    ...
    ta = transform_block(a, li, lj)
    tb = transform_block(b, lj, li)
