...
from maceh.data.io.blocks import key_str, matrices_to_blocks, parse_key, species_orbital_info, write_structure_files
from maceh.response.long_range import minimum_image_displacements
from maceh.data.io import abacus as abacus_io
from workflows.mgo_dataset import convert
from maceh.config import load_config, sha256_file
from maceh.response.constants import RY_TO_EV
from workflows.mgo_dataset.snapshot import SnapshotStore
from maceh.data.structures import make_supercell, rocksalt_primitive
from tests.integration.mgo_dataset.test_abacus_parsers import write_csr
...
def fabricate_dft(folder, cfg, sc, seed=0):
    ...
    types, norb, offsets = species_orbital_info(cfg, sc.species)
    dim = int(offsets[-1])
...
def test_matrices_to_blocks_units_and_transform(tmp_path):
    ...
    blocks = matrices_to_blocks(csr, dim, cfg, sc.species, RY_TO_EV)
    ...
    with pytest.raises(ValueError, match="dimension"):
        matrices_to_blocks(csr, dim, cfg, ["Mg"], 1.0)
...
def test_site_positions_are_wrapped_into_the_cell(tmp_path):
    ...
    assert np.abs(minimum_image_displacements(sc.cell, pos, sc.cart)
                  - u).max() < 1e-10
...
def test_undisplaced_positions_are_not_wrapped(tmp_path):
    ...
    folder = str(tmp_path)
    write_structure_files(folder, cell, cart, species, cfg, 0.0)
