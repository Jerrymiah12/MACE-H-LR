...
from maceh.response.constants import C_COUL, LR_SIGN
from maceh.response.long_range import assemble_lr_hamiltonian, blocks_diff_norm, blocks_norm, check_reciprocal_set, evaluate_potential, gmax_squared, imaginary_residual, lr_coefficients, minimum_image_displacements, reciprocal_set
from workflows.mgo_dataset.convert import key_str
...
def test_reciprocal_set_properties():
    cell, rec, vol = _cube()
    gmax_sq = gmax_squared(1.0, 1e-10)
    n_int, g_cart = reciprocal_set(rec, EPS_I, gmax_sq)
...
def test_check_flags_broken_set():
    n_int, _ = reciprocal_set(_cube()[1], EPS_I, gmax_squared(1.0, 1e-6))
    broken = n_int[1:]                      # drop one vector -> asymmetric
    assert check_reciprocal_set(broken)["ok"] is False
    with_zero = np.vstack([n_int, [0, 0, 0]])
    assert check_reciprocal_set(with_zero)["excludes_G_zero"] is False
...
def test_sign_and_prefactor_against_filtered_dipole():
    ...
    lam, tol = 0.6, 1e-8
    n_int, g_cart = reciprocal_set(rec, EPS_I, gmax_squared(lam, tol))
    dipoles = np.array([[0.0, 0.0, 0.1]])
    refpos = np.array([[4.0, 4.0, 4.0]])
    points = np.array([[4.0, 4.0, 5.0], [4.0, 4.0, 3.0], [5.0, 4.0, 4.0]])
    coeffs = lr_coefficients(g_cart, dipoles, refpos, EPS_I, lam, vol)
    v = evaluate_potential(g_cart, coeffs, points)
    ...
    assert imaginary_residual(v, 1e-12) < 1e-10
    vr = np.real(v)
...
def test_coefficients_linear_in_dipoles():
    cell, rec, vol = _cube()
    n_int, g = reciprocal_set(rec, EPS_I, gmax_squared(0.8, 1e-8))
    rng = np.random.default_rng(0)
    d = rng.standard_normal((4, 3)) * 0.01
    r0 = rng.uniform(0, 8, (4, 3))
    c1 = lr_coefficients(g, d, r0, EPS_I, 0.8, vol)
    c2 = lr_coefficients(g, 2.0 * d, r0, EPS_I, 0.8, vol)
    assert np.allclose(c2, 2.0 * c1)        # exactly linear in u_rel
    c0 = lr_coefficients(g, 0.0 * d, r0, EPS_I, 0.8, vol)
    assert np.allclose(c0, 0.0)             # equilibrium -> exact zero


def test_uniform_translation_exact_zero():
    from workflows.mgo_dataset.displacements import remove_uniform_translation
...
def test_realness_requires_inversion_symmetry():
    cell, rec, vol = _cube()
    n_int, g = reciprocal_set(rec, EPS_I, gmax_squared(0.8, 1e-6))
    d = np.array([[0.0, 0.0, 0.05]])
    r0 = np.array([[1.234, 2.345, 3.456]])
    c = lr_coefficients(g, d, r0, EPS_I, 0.8, vol)
    pts = np.array([[0.5, 1.5, 2.5]])
    assert imaginary_residual(evaluate_potential(g, c, pts), 1e-12) < 1e-10
    ...
    v_bad = evaluate_potential(g[mask], c[mask], pts)
    assert imaginary_residual(v_bad, 1e-12) > 1e-4
...
def test_gmax_convergence_at_fixed_lambda():
    ...
    for scale in (1.0, 1.5 ** 2):
        n_int, g = reciprocal_set(rec, EPS_I,
                                     gmax_squared(lam, tol) * scale)
        c = lr_coefficients(g, d, r0, EPS_I, lam, vol)
        vs.append(np.real(evaluate_potential(g, c, pts)))
...
def test_minimum_image_displacements():
    ...
    u = minimum_image_displacements(cell, cart, ref)
...
def test_assemble_and_hermiticity_and_small_amplitude():
    ...
    cell, rec, vol = _cube(8.0)
    lam = 0.8
    n_int, g = reciprocal_set(rec, EPS_I, gmax_squared(lam, 1e-8))
    ...
    def h_lr(amp):
        ...
        pos = ref + u
        c = lr_coefficients(g, d, ref, EPS_I, lam, vol)
        v = np.real(evaluate_potential(g, c, pos))
        ...
        return assemble_lr_hamiltonian(s, v)
    ...
    def e_sign(a):
        return blocks_diff_norm(h_lr(a), {k: -v for k, v in
                                             h_lr(-a).items()}) \
            / (blocks_norm(h_lr(a)) + delta)

    def e_linear(a):
        h2, h1 = h_lr(2 * a), h_lr(a)
        return blocks_diff_norm(h2, {k: 2.0 * v for k, v in h1.items()}) \
