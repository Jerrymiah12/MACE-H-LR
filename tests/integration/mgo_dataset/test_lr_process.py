...
from maceh.response.provenance import REFERENCE_DEFINITION_FILES

from maceh.data.io.blocks import key_str, parse_key, read_blocks
from maceh.response.long_range import blocks_diff_norm, blocks_norm
from workflows.mgo_dataset import convert, export, long_range
...
from tests.integration.mgo_dataset.test_convert import (Args, fabricate_dft, prepared_snapshot,
                                       small_cfg)
...
def test_lr_process_equilibrium_zero(tmp_path):
    ...
    assert blocks_diff_norm(total, h_full) < 1e-10
    ...
    assert set(fingerprints) == set(REFERENCE_DEFINITION_FILES)
...
def test_lr_process_translation_zero(tmp_path):
    ...
    long_range.lr_process_stage(cfg, ws, Args())
    h_lr = read_blocks(
        os.path.join(store.folder(sid), "hamiltonians_lr.h5"))
    assert blocks_norm(h_lr) < 1e-10        # exact zero by construction
...
def test_lr_process_idempotent_and_lambda_guard(tmp_path):
    ...
    long_range.lr_process_stage(cfg, ws, Args())               # skip, no --force
    ...
    with pytest.raises(SystemExit, match="lr_definition"):
        long_range.lr_process_stage(cfg2, ws, args)            # refuses to mix Λ
...
def test_lr_process_imaginary_gate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(long_range, "evaluate_potential", broken)
    assert long_range.lr_process_stage(cfg, ws, Args()) == 1
    folder = store.folder(sid)
...
def test_failed_forced_rerun_invalidates_old_labels_and_export(
    ...
    assert long_range.lr_process_stage(cfg, ws, force_args) == 1
    folder = store.folder(sid)
...
def test_unexpected_forced_rerun_error_leaves_no_stale_labels(
    ...
    monkeypatch.setattr(long_range, "evaluate_potential", crash)
    force_args = Args()
    force_args.force = True
    with pytest.raises(RuntimeError, match="synthetic evaluator crash"):
        long_range.lr_process_stage(cfg, ws, force_args)
    folder = store.folder(sid)
    for name in ("hamiltonians_lr.h5", "hamiltonians_sr.h5",
