...
from maceh.response.provenance import expected_lr_definition
from workflows.mgo_dataset import organize
...
from tests.integration.mgo_dataset.test_lr_process import add_dfpt_artifacts
...
def _write_lr_identity(ws, cfg):
    ...
    with open(os.path.join(ws, "metadata.yaml"), "w") as handle:
        yaml.safe_dump({"lr_definition":
                        expected_lr_definition(cfg, ws)}, handle)
...
def test_organize_stage(tmp_path):
    ...
    original_lr_definition = expected_lr_definition(cfg, ws)
...
def test_default_main_split_is_balanced_and_q_leakage_free():
    from workflows.mgo_dataset import displacements
    from maceh.data.structures import rocksalt_primitive
