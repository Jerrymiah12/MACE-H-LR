...
from maceh.analysis.locality import binned_norms, block_distance, controlled_q_comparisons, farfield_gate, farfield_sensitivity, long_range_localizes, odd_response, tail_fractions
from workflows.mgo_dataset import locality, validate
...
def test_block_distance():
    ...
    assert abs(block_distance("[0, 0, 0, 1, 2]", cart, cell) - 1.0) < 1e-12
    assert abs(block_distance("[1, 0, 0, 1, 1]", cart, cell) - 10.0) < 1e-12
    assert abs(block_distance("[-1, 0, 0, 1, 2]", cart, cell) - 9.0) < 1e-12
...
def test_tail_fractions_ordering():
    ...
    f = tail_fractions(blocks, cart, cell, [1.0, 5.0])
    total = 1.0 + 0.01
    assert abs(f[0] - 0.01 / total) < 1e-12
    assert f[1] == 0.0
    b = binned_norms(blocks, cart, cell, 1.0)
...
def test_odd_response_perfect_match():
    ...
    h_p = {k: np.array([[2.0]])}
    h_m = {k: np.array([[-2.0]])}
    out = odd_response(h_p, h_m, {k: np.array([[2.0]])}, 1e-12)
    assert abs(out["cos_theta"] - 1.0) < 1e-9
    assert abs(out["r_lr"] - 1.0) < 1e-9
    out2 = odd_response(h_p, h_m, {k: np.array([[-2.0]])}, 1e-12)
...
def test_locality_equal_tails_is_not_a_pass():
    ...
    assert long_range_localizes(f_full, list(f_full), 1e-6, 0.05) is False


def test_locality_measurable_improvement_passes():
    f_full = [0.9, 0.7, 0.5, 0.3]
    f_sr = [0.9, 0.7, 0.20, 0.10]          # >5% lower in the long-distance half
    assert long_range_localizes(f_full, f_sr, 1e-6, 0.05) is True
...
def test_controlled_q_comparisons_average_each_shell():
    ...
    comparisons = controlled_q_comparisons(metas, norms)
...
def test_farfield_gate_requires_every_far_bin_to_improve():
    ...
    ok, qualifying = farfield_gate(bins, 4.0, 1e-6, 20, 0.05)
    assert ok and len(qualifying) == 2        # near bins are not judged
    # one far bin below threshold fails the whole gate
    ok, _ = farfield_gate(bins + [_bin(6.0, 0.01)], 4.0, 1e-6, 20, 0.05)
    assert not ok
...
def test_farfield_gate_without_evidence_is_not_a_pass():
    assert farfield_gate([], 4.0, 1e-6, 20, 0.05)[0] is False
    # every candidate bin excluded -> still no pass
    assert farfield_gate([_bin(1.0, 0.9)], 4.0, 1e-6, 20, 0.05)[0] is False
