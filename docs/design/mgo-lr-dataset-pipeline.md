...
> Implementation note (2026-08-17): the reusable pieces described here now
> live under `maceh.data` and `maceh.response`; campaign stages are invoked as
> `python -m workflows.mgo_dataset`. Legacy `mgo_lr` names below describe the
> frozen metadata schema and pre-refactor layout.
...
```
python -m workflows.mgo_dataset init-reference | collect-reference
python -m workflows.mgo_dataset init-dfpt | collect-dfpt
python -m workflows.mgo_dataset gen-structures | collect-dft | lr-process | validate
                 | locality-report   --set pilot|main|large
python -m workflows.mgo_dataset organize
python -m workflows.mgo_dataset export-target --target full|lr|sr
python -m workflows.mgo_dataset status
```
...
`workflows/mgo_dataset/configs/mgo.yaml` is the single source of truth: lattice-constant
...
Pytest coverage now lives under `tests/integration/mgo_dataset/`; no DFT is run there:
