# MgO MACE-H-LR dataset-generation workflow
...
The workflow imports reusable numerics and I/O from `maceh`; the library never
imports this workflow. Compatibility with the MACE-H loader is a
...
```
python -m workflows.mgo_dataset <stage> --config workflows/mgo_dataset/configs/mgo.yaml --workspace <dir> [--set pilot|main|large] [--target full|lr|sr] [--force]
```
...
```
python -m pytest tests/integration/mgo_dataset -q
```
