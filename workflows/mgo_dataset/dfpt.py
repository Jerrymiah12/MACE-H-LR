...
def write_ph_input(path, cfg, *, trans=True, zeu=None):
    """Write a Gamma-point dielectric/Born-charge ``ph.x`` input.

    ``trans=False, zeu=True`` is the inexpensive tensor-only route: Born
    charges are evaluated as dF/dE from the three electric-field responses,
    without also solving every atomic-displacement representation needed for
    the dynamical matrix.  The default deliberately preserves the original
    ...
    lines = ["MgO q=0 DFPT: dielectric tensor and Born effective charges",
             ...
             "  epsil = .true.",
             f"  zeu = {'.true.' if zeu else '.false.'}",
             f"  trans = {'.true.' if trans else '.false.'}", "/",
             "0.0 0.0 0.0"]
    atomic_write_text(path, "\n".join(lines) + "\n")
...
def parse_ph_output(text):
    ...
    if eps is None or idx is None:
        raise ValueError(
            ...
            "charges — was ph.x run with epsil=.true. and zeu=.true.?")
