...
Usage
-----
    python -m workflows.training.check_split_wiring [--workspace DIR] [--training-root DIR]
    python -m workflows.training.check_split_wiring --configs-only   # checks 1-2 only
"""
import argparse
import ast
import os
    ...
    for subset, sids in (("train", train), ("validation", val)):
        for sid in sids:
            ...
            for name in sorted(os.listdir(src)):
                # Match make_trainval_view.py: preserve the logical symlink
                # path so an export-target switch propagates into this view.
...
def check_loader_membership(workspace):
    ...
    try:
        ...
        assert extra_val_loader is val_loader, \
            "identical validation memberships should reuse one loader/pass"
