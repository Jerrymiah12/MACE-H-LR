...
trains on is exactly what it is supposed to train on.
    ...
    [3] run the real `deephe3-train.py` as a subprocess, sampling GPU
...
def postflight(save_dir, device):
    import torch
    ...
    net = DeepHE3Kernel().load_model(os.path.join(save_dir, "src"),
    ...
    worst = max(float((ckpt["state_dict"][k].cpu() - again[k]).abs().max())
                for k in cmp)
    print(f"  reloaded into a fresh model from {save_dir}/src, "
...
def main():
    ...
    with open(log_path, "w") as log:
        proc = subprocess.run(
            [sys.executable, os.path.join(paths.REPO_ROOT, "deephe3-train.py"),
             config_path],
    ...
    losses = parse_losses(os.path.join(save_dir, "result.txt")) \
        if os.path.isfile(os.path.join(save_dir, "result.txt")) \
    ...
    postflight(save_dir, config.device)
