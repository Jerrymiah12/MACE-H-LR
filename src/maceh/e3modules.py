...
# The spherical Bessel basis originates in DimeNet, whose own repository is
# Hippocratic-2.1 licensed. PyTorch Geometric redistributes the same functions
# under MIT, so we take them from the dependency rather than vendoring a copy
# this project could not relicense. The two are symbolically identical.
# `dimenet_utils` is not part of PyG's documented API; tests/unit/
# test_radial_basis.py fails loudly if an upgrade moves or changes it.
...
from ._vendor.nequip.cutoffs import PolynomialCutoff
