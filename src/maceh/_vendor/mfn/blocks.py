...
from .utils import to_dense_adj, to_dense_batch
from .torch_tools import get_mask
...
@compile_mode("script")
class RealAgnosticResidualInteractionBlockUp(InteractionBlock):
    ...
    def _setup(self) -> None:
        ...
        # Linear used for expansion
        self.linear_up_expansion = o3.Linear(
            irreps_mid, self.irreps_out, internal_weights=True, shared_weights=True
        )
    ...
    def forward(
        ...
        message_out = self.linear(message) / self.avg_num_neighbors
        up = self.linear_up_expansion(message) / self.avg_num_neighbors
        return (
            self.reshape(message_out),
            sc,
            up
        )  # [n_nodes, channels, (lmax + 1)**2]
...
@compile_mode("script")
class RealAgnosticResidualInteractionBlockNorm(InteractionBlock):
    # adapted by Chen Qian
    def _setup(self) -> None:
        # First linear
        self.linear_up = o3.Linear(
        ...
        #e3LayerNorm
        self.norm = e3LayerNorm(irreps_mid)
    ...
    def forward(
        ...
        message = self.norm(message, batch)
        message = self.linear(message) #/ self.avg_num_neighbors
        return (
            self.reshape(message),
            sc,
...
@compile_mode("script")
class RealAgnosticResidualInteractionBlockUpNorm(InteractionBlock):
    # adapted by Chen Qian
    def _setup(self) -> None:
        # First linear
        self.linear_up = o3.Linear(
