...
class testResultAnalyzer:
    ...
    def error_plot(self, include=[], exclude=[], mode='mae'):
        ...
        # adapted by Chen Qian
        if mode not in ['mae', 'mse', 'rmse', 'rae']:
            print(f'Unknown mode: {mode}')
            return 1
        ...
        with h5py.File(self.h5file, 'r') as f:
            ...
            #adapted by Chen Qian
            if mode == 'rae':
                labels = [[[] for _ in range(num_element)] for _ in range(num_element)]
            ...
            for s in included:
                ...
                for i in range(num_element):
                    for j in range(num_element):
                        ...
                        # adapted by Chen Qian
                        if mode == 'rae':
                            label_s = label[selector]
                            label_s = self.kernel.update_hopping({}, label_s, node_attr, edge_index[:, selector], np.arange(selector.size), debug=True)
                            label_s = np.stack(list(label_s.values()), axis=0)
                            if self.dataset_info.spinful:
                                label_s = label_s.real
                            labels[i][j].append(label_s)        
        ...
        for i in range(num_element):
            for j in range(num_element):
                ...
                # adapted by Chen Qian
                if mode == 'rae':
                    std = np.std(np.concatenate(labels[i][j], axis=0), axis=0) + 1e-3 # 1e-3 for numerical stability
                    errors[i][j] /= std
                else:
        ...
        with open(os.path.join(output_dir, 'summary.txt'), 'w') as f:
            summary_info += '\n'.join(('----------',
                'Overall',
                'Avg: ' + str(np.mean(errors)),
                'Max: ' + str(np.amax(errors)),
                'Min: ' + str(np.amin(errors)),
                '\n'))
    ...
    def std_error_analysis(self, include=[], exclude=[], mode='mae'):
        # if mode not in ['mae', 'mse', 'rmse']:
        #     print(f'Unknown mode: {mode}')
        #     return 1
        ...
        with h5py.File(self.h5file, 'r') as f:
            ...
            for s in included:
                ...
                #adapted by Chen Qian
                errors_s = [[None for _ in range(num_element)] for _ in range(num_element)]
                ...
                for i in range(num_element):
                    for j in range(num_element):
                        ...
                        #adapted by Chen Qian
                        errors_s[i][j] = copy.deepcopy(error)

                #adapted by Chen Qian
                errors_s_list.append(errors_s)   
        ...
        #adapted by Chen Qian
        for s_idx in range(len(included)):
            for i in range(num_element):
                for j in range(num_element):
                    # errors[i][j] /= n
                    if mode == 'rmse':
                        errors_s_list[s_idx][i][j] = np.sqrt(errors_s_list[s_idx][i][j])

                    # adapted by Chen Qian
                    if mode == 'rae':
                        std = np.std(np.concatenate(labels[i][j], axis=0), axis=0) + 1e-3 # 1e-3 for numerical stability
                        errors_s_list[s_idx][i][j] /= std
                    else:
                        errors_s_list[s_idx][i][j] *= 1000 # convert to meV
        ...
        summary_info += '\n'.join(('----------',
                                    'Overall_Std',
                                    'Avg_Std: ' + str(np.std([np.mean(np.block(i)) for i in errors_s_list])),
                                    'Max_Std: ' + str(np.std([np.amax(np.block(i)) for i in errors_s_list])),
                                    'Min_Std: ' + str(np.std([np.amin(np.block(i)) for i in errors_s_list])),
                                    '\n'))
        with open(os.path.join(output_dir, 'summary.txt'), 'w') as f:
            f.write(summary_info)
        print('Written summary to summary.txt')
...
with (files('maceh.data') / 'periodic_table.json').open('r') as f:
    elem = json.load(f)
