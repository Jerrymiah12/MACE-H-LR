import os
import time
import ast
import hashlib
from importlib.resources import files
from configparser import ConfigParser
import numpy as np

import torch

from e3nn.o3 import Irreps

from .utils import orbital_analysis, refine_post_node

class BaseConfig:
    def __init__(self, config_file=None):
        
        self._config = ConfigParser(inline_comment_prefixes=(';',))
        
        if self.__class__ is __class__:
            base_default = str(files('maceh.default_configs') / 'base_default.ini')
            print(f'Getting config file from: {config_file}')
            self.get_config(config_file, config_file_default=base_default)
            
            self.get_basic_section()
            self.get_data_section()
    
    def get_config(self, config_file, config_file_default=''):
        if config_file_default:
            assert os.path.isfile(config_file_default)
            self._config.read(config_file_default)
        assert os.path.isfile(config_file)
        self.config_file = config_file
        self._config.read(config_file)
    
    def get_data_section(self):
        self.graph_dir = self._config.get('data', 'graph_dir')

        self.dft_data_dir = self._config.get('data', 'DFT_data_dir')
        self.processed_data_dir = self._config.get('data', 'processed_data_dir')
        
        self.save_graph_dir = self._config.get('data', 'save_graph_dir')
        self.target_data = self._config.get('data', 'target_data')
        self.dataset_name = self._config.get('data', 'dataset_name')
        
        self.get_olp = self._config.getboolean('data', 'get_overlap')
        
        self.radius = self._config.getfloat('data', 'radius')
        
        self.only_ij = False # todo
    
    def set_dtype(self, dtype):
        if dtype == 'float':
            self.torch_dtype = torch.float32
            self.np_dtype = np.float32
        elif dtype == 'double':
            self.torch_dtype = torch.float64
            self.np_dtype = np.float64
        else:
            raise NotImplementedError
    
    def get_basic_section(self):
        self.device = torch.device(self._config.get('basic', 'device'))
        dtype = self._config.get('basic', 'dtype')
        self.set_dtype(dtype)
        self.inference = self._config.getboolean('basic', 'inference')

class TrainConfig(BaseConfig):
    def __init__(self, config_file):
        super().__init__()
        train_default = str(files('maceh.default_configs') / 'train_default.ini')

        print(f'Loading train config from: {config_file}')
        self.get_config(config_file, config_file_default=train_default)
        
        self.get_basic_section()
        self.get_data_section()
        self.get_train_section()
        
        if self.use_new_hypp:
            self.get_hypp_section()
        
        if self.checkpoint_dir:
            # Tensor heads are a new wrapper around an old SR checkpoint.  Its
            # saved defaults may contain a disabled [tensors] section, so retain
            # the requesting config's tensor workflow while importing only the
            # legacy hyperparameter/target/network definitions documented here.
            tensor_options = dict(self._config.items('tensors')) \
                if self._config.has_section('tensors') else None
            assert self.checkpoint_dir.endswith('.pkl'), 'checkpoint should point to model.pkl or best_model.pkl'
            config1_path = os.path.join(os.path.dirname(self.checkpoint_dir), 'src/train.ini')
            assert os.path.isfile(config1_path), 'cannot find train.ini under checkpoint dir'
            print(f'Overwriting sections [hyperparameters], [target] and [network] using train config in checkpoint: {config1_path}')
            self.get_config(config1_path, config_file_default=train_default)
            if tensor_options is not None:
                self._config.read_dict({'tensors': tensor_options})
        
        # overwrite settings using those in config
        if not self.use_new_hypp:
            self.get_hypp_section()
        
        self.get_target_section()
        self.get_network_section()
        self.get_tensor_section()
        
        self._target_set_flag = False
        # set_target should be called once dataset has been prepared
        
    def get_basic_section(self):
        super().get_basic_section()
        self.float32_matmul_precision = self._config.get(
            'basic', 'float32_matmul_precision', fallback='highest').lower()
        valid_matmul_precision = {'highest', 'high', 'medium'}
        if self.float32_matmul_precision not in valid_matmul_precision:
            raise ValueError(
                'float32_matmul_precision must be one of '
                f'{sorted(valid_matmul_precision)}, got '
                f'{self.float32_matmul_precision!r}')
        self.seed = self._config.getint('basic', 'seed')
        self.checkpoint_dir = self._config.get('basic', 'checkpoint_dir')
        self.simp_out = self._config.getboolean('basic', 'simplified_output')
        self.use_new_hypp = self._config.getboolean('basic', 'use_new_hypp')

        # = save to time folder =
        save_dir = self._config.get('basic', 'save_dir')
        additional_folder_name = self._config.get('basic', 'additional_folder_name')
        self.save_dir = os.path.join(save_dir, str(time.strftime('%Y-%m-%d_%H-%M-%S', time.localtime(time.time()))))
        if additional_folder_name:
            self.save_dir = self.save_dir + '_' + additional_folder_name
        assert not os.path.exists(self.save_dir)
        
    def get_train_section(self):
        self.batch_size = self._config.getint('train', 'batch_size')
        self.num_epoch = self._config.getint('train', 'num_epoch')

        self.checkpoint_interval = self._config.getint(
            'train', 'checkpoint_interval', fallback=1)
        if self.checkpoint_interval < 1:
            raise ValueError('checkpoint_interval must be at least 1')

        self.early_stop_val_loss = self._config.getfloat(
            'train', 'early_stop_val_loss', fallback=-1.0)
        self.early_stop_min_epochs = self._config.getint(
            'train', 'early_stop_min_epochs', fallback=0)
        self.early_stop_patience = self._config.getint(
            'train', 'early_stop_patience', fallback=1)
        if not np.isfinite(self.early_stop_val_loss):
            raise ValueError('early_stop_val_loss must be finite')
        if self.early_stop_min_epochs < 0:
            raise ValueError('early_stop_min_epochs cannot be negative')
        if self.early_stop_patience < 1:
            raise ValueError('early_stop_patience must be at least 1')
        
        self.min_lr = self._config.getfloat('train', 'min_lr')
            
        ev = self._config.get('train', 'extra_validation')
        self.extra_val = eval(ev) if ev else []
        self.extra_val_test_only = self._config.getboolean('train', 'extra_val_test_only')
        
        self.train_ratio = self._config.getfloat('train', 'train_ratio')
        self.val_ratio = self._config.getfloat('train', 'val_ratio')
        self.test_ratio = self._config.getfloat('train', 'test_ratio')
        
        self.train_size = self._config.getint('train', 'train_size')
        self.val_size = self._config.getint('train', 'val_size')
        self.test_size = self._config.getint('train', 'test_size')
    
    def get_hypp_section(self):
        self.lr = self._config.getfloat('hyperparameters', 'learning_rate')
        self.adam_betas = eval(self._config.get('hyperparameters', 'Adam_betas'))

        self.scheduler_type = self._config.getint('hyperparameters', 'scheduler_type')
        ts = self._config.get('hyperparameters', 'scheduler_params')
        if ts:
            ts = 'dict' + ts
            self.scheduler_params = eval(ts)
        else:
            self.scheduler_params = dict()
        
        self.revert_decay_patience = self._config.getint('hyperparameters', 'revert_decay_patience')
        self.revert_decay_rate = self._config.getfloat('hyperparameters', 'revert_decay_rate')
        
    def get_target_section(self):
        # target
        self.target = self._config.get('target', 'target')
        tbt = self._config.get('target', 'target_blocks_type')
        assert len(tbt) > 0, 'Invalid target_blocks_type'
        self.tbt0 = tbt[0].lower()
        self._target_blocks = None
        if self.tbt0 == 's':
            self._target_blocks = eval(self._config.get('target', 'target_blocks'))
        sep = self._config.get('target', 'selected_element_pairs')
        self.element_pairs = eval(sep) if sep else None
        self.convert_net_out = self._config.getboolean('target', 'convert_net_out')
        assert not self.convert_net_out
    
    def get_network_section(self):
        # network
        self.cutoff_radius = self._config.getfloat('network', 'cutoff_radius')
        self.only_ij = self._config.getboolean('network', 'only_ij')
        assert not self.only_ij
        self.no_parity = self._config.getboolean('network', 'ignore_parity')
        
        sh_lmax = self._config.get('network', 'spherical_harmonics_lmax')
        sbf_irreps = self._config.get('network', 'spherical_basis_irreps')
        if sh_lmax:
            assert not sbf_irreps, 'spherical_harmonics_lmax and spherical_basis_irreps cannot be provided simultaneously'
            sh_lmax = int(sh_lmax)
            self.irreps_sh = Irreps([(1, (i, 1 if self.no_parity else (-1) ** i)) for i in range(sh_lmax + 1)])
            self.use_sbf = False
        else:
            assert sbf_irreps, 'at least one of spherical_harmonics_lmax and spherical_basis_irreps should be provided'
            self.irreps_sh = Irreps(sbf_irreps)
            self.use_sbf = True
        
        irreps_mid = self._config.get('network', 'irreps_mid')
        if irreps_mid:
            self.irreps_mid_node = irreps_mid
            self._irreps_post_node = irreps_mid
            self.irreps_mid_edge = irreps_mid
        irreps_embed = self._config.get('network', 'irreps_embed')
        if irreps_embed:
            self.irreps_embed_node = irreps_embed
            self.irreps_edge_init = irreps_embed
        if self.target in ['hamiltonian']:
            self.irreps_out_node = '1x0e'
        self.num_blocks = self._config.getint('network', 'num_blocks')
        
        # ! post edge
        for name in ['irreps_embed_node', 'irreps_edge_init', 'irreps_mid_node', 'irreps_out_node', 'irreps_mid_edge']:
            irreps = self._config.get('network', name)
            if irreps:
                delattr(self, name)
                setattr(self, name, irreps)
        
        self._net_out_irreps = self._config.get('network', 'out_irreps')
        self._irreps_post_edge = self._config.get('network', 'irreps_post_edge')

        # what I added for developement, by Chen Qian
        self.num_bessel = self._config.getint('network', 'num_bessel')
        self.num_polynomial_cutoff = self._config.getint('network', 'num_polynomial_cutoff')
        self.max_ell = self._config.getint('network', 'max_ell')
        self.hidden_irreps = self._config.get('network', 'hidden_irreps')
        self.avg_num_neighbors = eval(self._config.get('network', 'avg_num_neighbors'))
        self.correlation = self._config.getint('network', 'correlation')
        self.radial_MLP = eval(self._config.get('network', 'radial_MLP'))
        self.mace_norm = self._config.get('network', 'mace_norm')
        self.expand_nonlin = self._config.getboolean('network', 'expand_nonlin')
        self.expand_norm = self._config.get('network', 'expand_norm')
        self.basis_func = self._config.get('network', 'basis_func')
        self.num_gaussian = self._config.get('network', 'num_gaussian')
        self.shift_scale = self._config.getboolean('network', 'shift_scale')

    def get_tensor_section(self):
        section = 'tensors'
        getbool = lambda key, fallback=False: self._config.getboolean(
            section, key, fallback=fallback)
        getfloat = lambda key, fallback: self._config.getfloat(
            section, key, fallback=fallback)
        self.predict_born = getbool('predict_born')
        self.predict_epsilon = getbool('predict_epsilon')
        if self.predict_born != self.predict_epsilon:
            raise ValueError('Born and dielectric heads must be enabled together')
        self.tensor_enabled = self.predict_born and self.predict_epsilon
        self.tensor_manifest = self._config.get(
            section, 'manifest', fallback='').strip()

        def ids(key):
            value = self._config.get(section, key, fallback='').strip()
            parsed = ast.literal_eval(value) if value else []
            if not isinstance(parsed, (list, tuple)) or not all(
                    isinstance(item, str) for item in parsed):
                raise ValueError(f'[tensors] {key} must be a list of snapshot IDs')
            return list(parsed)

        self.tensor_train_ids = ids('train_ids')
        self.tensor_validation_ids = ids('validation_ids')
        self.tensor_test_ids = ids('test_ids')
        self.born_loss_weight = getfloat('born_loss_weight', 1.0)
        self.epsilon_loss_weight = getfloat('epsilon_loss_weight', 1.0)
        self.backbone_lr = getfloat('backbone_learning_rate', self.lr)
        self.tensor_head_lr = getfloat('head_learning_rate', self.lr)
        self.h_head_lr = getfloat('h_head_learning_rate', self.backbone_lr)
        self.freeze_backbone = getbool('freeze_backbone')
        self.freeze_h_head = getbool('freeze_h_head')
        self.unfreeze_last_interaction_blocks = self._config.getint(
            section, 'unfreeze_last_interaction_blocks', fallback=0)
        self.aux_gradient_max_norm = getfloat('aux_gradient_max_norm', -1.0)
        self.tensor_label_repeat = self._config.getint(
            section, 'labelled_structure_repeat', fallback=4)
        self.tensor_checkpoint_mode = self._config.get(
            section, 'checkpoint_mode', fallback='resume').strip().lower()
        self.dielectric_eigenvalue_floor = getfloat(
            'dielectric_eigenvalue_floor', 1.0e-4)
        self.h_validation_gate = getfloat('h_validation_gate', 3.0e-6)
        self.born_validation_mae_gate = getfloat(
            'born_validation_mae_gate', 0.002778)
        self.epsilon_validation_mae_gate = getfloat(
            'epsilon_validation_mae_gate', 0.001532)
        self.multitask_gate_patience = self._config.getint(
            section, 'multitask_gate_patience', fallback=10)
        if self.tensor_enabled:
            if not self.tensor_manifest:
                raise ValueError('[tensors] manifest is required')
            if not os.path.isfile(self.tensor_manifest):
                raise FileNotFoundError(
                    f'tensor manifest does not exist: {self.tensor_manifest}')
            if not self.tensor_train_ids or not self.tensor_validation_ids:
                raise ValueError('tensor train_ids and validation_ids are required')
            all_ids = (self.tensor_train_ids + self.tensor_validation_ids
                       + self.tensor_test_ids)
            if len(all_ids) != len(set(all_ids)):
                raise ValueError('tensor train/validation/test IDs overlap')
            if min(self.born_loss_weight, self.epsilon_loss_weight) < 0:
                raise ValueError('tensor loss weights must be nonnegative')
            if min(self.backbone_lr, self.tensor_head_lr, self.h_head_lr) <= 0:
                raise ValueError('all multitask learning rates must be positive')
            if self.unfreeze_last_interaction_blocks < 0:
                raise ValueError('unfreeze_last_interaction_blocks cannot be negative')
            if self.tensor_label_repeat < 1:
                raise ValueError('labelled_structure_repeat must be at least one')
            if self.tensor_checkpoint_mode not in {'initialize', 'resume'}:
                raise ValueError('checkpoint_mode must be initialize or resume')
            with open(self.tensor_manifest, 'rb') as handle:
                self.tensor_manifest_sha256 = hashlib.sha256(
                    handle.read()).hexdigest()
        else:
            self.tensor_manifest_sha256 = ''

    @property
    def checkpoint_metadata(self):
        if not self.tensor_enabled:
            return {}
        return {
            'model_type': 'sr_born_epsilon',
            'direct_full_h_head': False,
            'analytic_lr_reconstruction': True,
            'tensor_label_manifest': {
                'path': os.path.abspath(self.tensor_manifest),
                'sha256': self.tensor_manifest_sha256,
            },
        }
    
    def set_target(self, orbital_types, index_to_Z, spinful, output_file):
        atom_orbitals = {}
        for Z, orbital_type in zip(index_to_Z, orbital_types):
            atom_orbitals[str(Z.item())] = orbital_type
            
        target_blocks, net_out_irreps, irreps_post_edge = orbital_analysis(atom_orbitals, self.tbt0, spinful, targets=self._target_blocks, 
                                                                           element_pairs=self.element_pairs, no_parity=self.no_parity, verbose=output_file)
        
        self._target_blocks = target_blocks
        if not self._net_out_irreps:
            self._net_out_irreps = net_out_irreps
        if not self._irreps_post_edge:
            self._irreps_post_edge = irreps_post_edge
            
        if self.convert_net_out:
            self._net_out_irreps = Irreps(self._net_out_irreps).sort().irreps.simplify()
            
        if spinful:
            if_verbose = False
            if output_file:
                if_verbose = True
            self._irreps_post_node = refine_post_node(irreps_post_node=self._irreps_post_node,
                                                      irreps_mid_node=self.irreps_mid_node,
                                                      irreps_mid_edge=self.irreps_mid_edge,
                                                      irreps_sh=self.irreps_sh,
                                                      irreps_post_edge=self._irreps_post_edge,
                                                      if_verbose=if_verbose)
            
        self._target_set_flag = True
    
    @property
    def target_blocks(self):
        assert self._target_set_flag
        return self._target_blocks
    
    @property
    def net_out_irreps(self):
        assert self._target_set_flag
        return self._net_out_irreps
    
    @property
    def irreps_post_edge(self):
        assert self._target_set_flag
        return self._irreps_post_edge
    
    @property
    def irreps_post_node(self):
        assert self._target_set_flag
        ipn = self._config.get('network', 'irreps_post_node')
        if ipn:
            return ipn
        else:
            return self._irreps_post_node
    
            
class EvalConfig(BaseConfig):
    def __init__(self, config_file):
        super().__init__()
        eval_default = str(files('maceh.default_configs') / 'eval_default.ini')
        
        print(f'Loading evaluation config from: {config_file}')
        self.get_config(config_file, config_file_default=eval_default)
        
        self.get_basic_section()
        self.get_data_section()
        
    def get_basic_section(self):
        # basic
        self.model_dir = self._config.get('basic', 'trained_model_dir')
        super().get_basic_section()
        self.out_dir = self._config.get('basic', 'output_dir')
        os.makedirs(self.out_dir, exist_ok=True)
        self.target = self._config.get('basic', 'target')
        self.inference = self._config.getboolean('basic', 'inference')
        self.test_only = self._config.getboolean('basic', 'test_only')
        if self.test_only: assert not self.inference


class EPCConfig(EvalConfig):
    """Configuration for Cartesian-AO finite-difference EPC inference."""

    def __init__(self, config_file):
        BaseConfig.__init__(self)
        epc_default = str(files('maceh.default_configs') / 'epc_default.ini')
        print(f'Loading EPC config from: {config_file}')
        self.get_config(config_file, config_file_default=epc_default)

        self.get_basic_section()
        self.get_data_section()
        self.get_epc_section()

    def get_epc_section(self):
        self.structure_dir = self._config.get('epc', 'structure_dir')
        self.q_grid = tuple(int(x) for x in
                            self._config.get('epc', 'q_grid').split())
        self.k_grid = tuple(int(x) for x in
                            self._config.get('epc', 'k_grid').split())
        assert len(self.q_grid) == 3 and len(self.k_grid) == 3
        assert all(n > 0 for n in self.q_grid + self.k_grid), \
            'q_grid and k_grid entries must be positive integers'
        self.delta = self._config.getfloat('epc', 'delta')
        assert np.isfinite(self.delta) and self.delta > 0, \
            'delta must be a positive finite displacement (Angstrom)'
        self.grad_threshold = self._config.getfloat('epc', 'grad_threshold')
        assert np.isfinite(self.grad_threshold) and self.grad_threshold >= 0, \
            'grad_threshold must be a nonnegative finite number (eV/Angstrom)'
        atom_indices = self._config.get('epc', 'atom_indices')
        self.atom_indices = (
            list(dict.fromkeys(int(x) for x in atom_indices.split()))
            if atom_indices.strip() else None)
        self.save_derivatives = self._config.getboolean(
            'epc', 'save_derivatives')
        self.analytic_lr_workspace = self._config.get(
            'epc', 'analytic_lr_workspace', fallback='').strip()
        self.analytic_lr_overlap_dir = self._config.get(
            'epc', 'analytic_lr_overlap_dir', fallback='').strip()
        self.analytic_lr_config = self._config.get(
            'epc', 'analytic_lr_config', fallback='').strip()
        self.analytic_lr_tensor_source = self._config.get(
            'epc', 'analytic_lr_tensor_source', fallback='reference').strip().lower()
        self.analytic_lr_tensor_mode = self._config.get(
            'epc', 'analytic_lr_tensor_mode',
            fallback='equilibrium_frozen').strip().lower()
        self.gauge_overlap_dir = self._config.get(
            'epc', 'gauge_overlap_dir', fallback='').strip()
        lr_fields = (self.analytic_lr_workspace,
                     self.analytic_lr_overlap_dir,
                     self.analytic_lr_config)
        if any(lr_fields) and not all(lr_fields):
            raise ValueError(
                'analytic_lr_workspace, analytic_lr_overlap_dir, and '
                'analytic_lr_config must be set together')
        if self.analytic_lr_tensor_source not in {'reference', 'model'}:
            raise ValueError('analytic_lr_tensor_source must be reference or model')
        if self.analytic_lr_tensor_mode not in {
                'equilibrium_frozen', 'geometry_dependent'}:
            raise ValueError('analytic_lr_tensor_mode must be '
                             'equilibrium_frozen or geometry_dependent')
        assert self.inference, 'EPC requires inference = True'
        assert self.radius > 0, \
            'EPC requires [data] radius > 0 (supercell graph cutoff)'
