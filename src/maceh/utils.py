...
from ._vendor.nequip.tp_utils import tp_path_exists
...
class LossThresholdStopper:
    """Stop after a stable absolute validation-loss target is reached."""

    def __init__(self, threshold=-1.0, min_epochs=0, patience=1):
        if min_epochs < 0:
    ...
    def step(self, epoch, val_loss, observation_count=1):
        # An empty LossRecord has avg == 0, so it must not look like a perfect
        # validation result. Non-finite observations also reset the streak.
        val_loss = float(val_loss)
        self.last_qualified = (
            self.enabled
            and observation_count > 0
            and np.isfinite(val_loss)
            and val_loss <= self.threshold
        )
        if self.last_qualified:
            self.consecutive += 1
        else:
            self.consecutive = 0
        return (
            ...
            and self.consecutive >= self.patience
        )

    def state_dict(self):
        # The active config owns the policy on resume. Store its signature so
        # a streak accumulated under a different threshold is never reused.
        return {
    ...
    def load_state_dict(self, state_dict):
        ...
        saved_policy = (
            state_dict.get('threshold', self.threshold),
            state_dict.get('min_epochs', self.min_epochs),
            state_dict.get('patience', self.patience),
        )
        active_policy = (self.threshold, self.min_epochs, self.patience)
        if saved_policy == active_policy:
            self.consecutive = max(
                0, int(state_dict.get('consecutive', 0)))
        else:
            self.consecutive = 0
        self.last_qualified = False
...
class RevertDecayLR:
    def __init__(self, model, optimizer, save_model_dir, decay_patience=20,
                 decay_rate=0.8, scheduler_type=0, scheduler_params=None,
        ...
        if scheduler_type == 1:
            ...
            # PyTorch 2.13 removed the deprecated ``verbose`` argument.  Keep
            # accepting existing MACE-H configs across torch versions; epoch
            # records already report every learning-rate change.
            scheduler_signature = inspect.signature(ReduceLROnPlateau)
            if 'verbose' not in scheduler_signature.parameters:
    ...
    def state_dict(self):
        state_dict = {key: value for key, value in self.__dict__.items() 
                      if key not in ['model', 'optimizer', 'save_model_dir',
                                     'scheduler', 'checkpoint_interval',
                                     '_last_latest_epoch']}
    ...
    def reconcile_best(self, epoch, val_loss):
        """Preserve a best checkpoint newer than a periodic restart file."""
        if val_loss < self.best_loss:
            self.best_loss = float(val_loss)
            self.best_epoch = int(epoch)
    
    def step(self, val_loss, **checkpoint_state):
        epoch = self.next_epoch
        self.next_epoch += 1
        ...
        if self.bad_epochs >= self.decay_patience:
            ...
            # Loading an old best checkpoint must not rewind the physical
            # epoch counter; that can otherwise create an unbounded loop.
            self.revert(preserve_next_epoch=self.next_epoch)
            self.decay()
    ...
    def revert(self, preserve_next_epoch=None):
        ...
        if preserve_next_epoch is not None:
            self.next_epoch = preserve_next_epoch
    ...
    def save_model(self, epoch, val_loss, is_best=False,
                   force_latest=False, **kwargs):
        save_latest = (
            force_latest
            or epoch == 0
            or (epoch + 1) % self.checkpoint_interval == 0
        )
        if not is_best and not save_latest:
            return
        ...
        state.update(kwargs)
        latest_path = os.path.join(self.save_model_dir, 'model.pkl')
        best_path = os.path.join(self.save_model_dir, 'best_model.pkl')
        if save_latest:
            self._atomic_torch_save(state, latest_path)
            self._last_latest_epoch = epoch
        if is_best:
            if save_latest:
                self._atomic_copy(latest_path, best_path)
            else:
                self._atomic_torch_save(state, best_path)

    def save_latest(self, epoch, val_loss, **kwargs):
        """Force a restart checkpoint after a fully completed final epoch."""
        if self._last_latest_epoch == epoch:
            return
        self.save_model(epoch, val_loss, force_latest=True, **kwargs)
